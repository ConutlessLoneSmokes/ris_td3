...
class SingleCritic(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.state_branch = _linear_block(state_dim, state_hidden_dim, use_layer_norm)
        self.action_branch = _linear_block(action_dim, action_hidden_dim, use_layer_norm)
        dims = (state_hidden_dim,) + tuple(hidden_dims)
        layers: list[nn.Module] = []
        for in_dim, out_dim in zip(dims[:-1], dims[1:]):
            layers.append(_linear_block(in_dim, out_dim, use_layer_norm))
        layers.append(nn.Linear(dims[-1], 1))
    ...
    def forward(self, state: torch.Tensor, action: torch.Tensor) -> torch.Tensor:
        fused = self.state_branch(state) + self.action_branch(action)
